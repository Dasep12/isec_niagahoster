...
class Historis extends CI_Controller
{
    ...
    public function index(Type $var = null)
    {
        ...
        $data = [
            ...
            DATE_FORMAT(pl.date_lembur,'%Y-%m')= '" . date('Y-m') . "' AND   (pl.wilayah = '" . $wil . "' OR pl.wilayah = '" . $wil_ . "' ) AND 
            ...
            WHERE DATE_FORMAT(c.tanggal_cuti,'%Y-%m')= '" . date('Y-m') . "' AND (c.wilayah='" . $wil . "' OR c.wilayah='" . $wil_ . "'  )
             AND c.status != 0  AND b.npk = c.npk AND b.id_biodata = e.id_employee"),

            'skta'      =>  $this->db->query("SELECT p.id , b.nama , p.npk , p.keterangan ,  p.date_in , p.in_time , p.out_time , p.date_out , p.area  , p.status , e.area_kerja
            FROM pengajuan_skta p , biodata b  , employee e 
            WHERE 
            DATE_FORMAT(p.date_in,'%Y-%m')= '" . date('Y-m') . "' AND (e.wilayah='" . $wil . "' OR e.wilayah='" . $wil_ . "'   ) AND
            e.npk = p.npk
            ...
            WHERE 
            DATE_FORMAT(pp.date_perijinan,'%Y-%m')= '" . date('Y-m') . "' AND (pp.wilayah='" . $wil . "' OR pp.wilayah='" . $wil_ . "'  )AND
            pp.npk = b.npk AND b.id_biodata = e.id_employee AND pp.status != 0  ")
        ];
        $this->load->view("web/spv/header", $data);
        $this->load->view("SPV/approval/log_approval", $data);
